...
%type<node> ABSTRACT_KEYWORD CONTINUE_KEYWORD FOR_KEYWORD NEW_KEYWORD ASSERT_KEYWORD IF_KEYWORD PACKAGE_KEYWORD SYNCHRONIZED_KEYWORD BOOLEAN_KEYWORD DO_KEYWORD PRIVATE_KEYWORD THIS_KEYWORD BREAK_KEYWORD DOUBLE_KEYWORD PROTECTED_KEYWORD EXTENDS_KEYWORD THROW_KEYWORD BYTE_KEYWORD ELSE_KEYWORD PUBLIC_KEYWORD INSTANCEOF_KEYWORD RETURN_KEYWORD TRANSIENT_KEYWORD INT_KEYWORD SHORT_KEYWORD CHAR_KEYWORD FINAL_KEYWORD STATIC_KEYWORD VOID_KEYWORD CLASS_KEYWORD LONG_KEYWORD STRICTFP_KEYWORD VOLATILE_KEYWORD FLOAT_KEYWORD NATIVE_KEYWORD SUPER_KEYWORD WHILE_KEYWORD
...
%type<node> additive_expression and_expression argument_list argument_list_zero_or_one array_access array_creation_expression array_initializer array_type assert_statement assignment assignment_expression assignment_operators basic_for_statement basic_for_statement_no_short_if block block_statement block_statements block_statements_zero_or_more block_statements_zero_or_one break_statement class_body class_body_declaration class_body_declaration_zero_or_more class_body_zero_or_one class_declaration class_extends class_extends_zero_or_one class_instance_creation_expression class_literal class_member_declaration class_type comma_expression_zero_or_more comma_statement_expression_zero_or_more comma_variable_declarator_zero_or_more commas_zero_or_more compilation_unit conditional_and_expression conditional_or_expression condtional_expression constructor_body constructor_declaration constructor_declarator continue_statement dim_expr dim_exprs dims dims_zero_or_one do_statememt empty_array_one_or_more empty_statement enhanced_for_statement enhanced_for_statement_no_short_if equality_expression exclusive_or_expression explicit_constructor_invocation expression expression_statement expression_zero_or_one field_access field_declaration for_init for_init_zero_or_one for_statement for_statement_no_short_if for_update for_update_zero_or_one identifier_dot_zero_or_one identifier_zero_or_one if_then_else_statement if_then_else_statement_no_short_if if_then_statement inclusive_or_expression instance_of_expression labeled_statement labeled_statement_no_short_if local_class_or_interface_declaration local_variable_declaration local_variable_declaration_statement method_invocation method_reference  multiplicative_expression normal_class_declaration ordinary_compilation_unit package_declaration pattern post_decrement_expression post_increment_expression postfix_expression pre_decrement_expression pre_increment_expression primary primary_no_new_array reciever_parameter reference_type relational_expression return_statement shift_expression start_state statement statement_expression statement_expression_list statement_no_short_if statement_without_trailing_substatement static_initializer synchronized_statement throw_statement top_level_class_or_interface_declaration top_level_class_or_interface_declaration_zero_or_more  type_name type_pattern unary_expression unary_expression_not_plus_minus unqualified_class_instance_creation_expression variable_declarator variable_declarator_list variable_initializer variable_initializer_list variable_initializer_list_zero_or_more while_statement while_statement_no_short_if
...
%type<method_declaration> method_declaration method_header method_declarator
%type<variable_declarator_id>variable_declarator_id
...
//             |   FINAL_KEYWORD lambda_parameter_type variable_declarator_id                                                              {Node* node = createNode("lambda parameter"); node->addChildren({$1,$2,$3}); $$ = node;}
...
//         |   FINAL_KEYWORD catch_type variable_declarator_id                                                                             {Node* node = createNode("catch formal parameter"); node->addChildren({$1,$2,$3}); $$ = node;}
...
// record_declaration
//         :  modifiers_zero_or_more  IDENTIFIERS type_parameters_zero_or_one record_header class_implements_zero_or_one record_body         {Node* node = createNode("record declaration"); node->addChildren({$1,$2,$3,$4,$5,$6,$7}); $$ = node;}
...
normal_class_declaration
        :  modifiers_zero_or_more CLASS_KEYWORD IDENTIFIERS class_extends_zero_or_one class_body                                                          {NormalClassDeclaration* node = new NormalClassDeclaration("normal class declaration", $1, $3->lexeme); node->line_no = $2->line_no; node->entry_type = CLASS_DECLARATION; node->addChildren({$1,$2,$3,$4,$5}); ((LocalSymbolTable*)((global_symtab->symbol_tables)[global_symtab->current_level.first][global_symtab->current_level.second]))->add_entry(node); $$ = node;}
...
variable_declarator
        :  variable_declarator_id                                                                                                                       {Node* node = createNode("variable declarator"); node->addChildren({$1}); $$ = node;}
        |  variable_declarator_id ASSIGNMENT_OP variable_initializer                                                                                    {Node* node = createNode("variable declarator"); node->addChildren({$1,$2,$3}); $$ = node;}

variable_declarator_id
        :  IDENTIFIERS dims_zero_or_one                                                                                                                 {VariableDeclaratorId* node = new VariableDeclaratorId("variable declarator id", $1->lexeme, 0); node->addChildren({$1,$2}); $$ = node;}
...
method_declaration
        :  modifiers_zero_or_more method_header block                                                                                                   {MethodDeclaration* node($2); node->modifiers = $1; node->addChildren({$1,$2,$3}); node->entry_type = METHOD_DECLARATION; ((LocalSymbolTable*)((global_symtab->symbol_tables)[global_symtab->current_level.first][global_symtab->current_level.second]))->add_entry(node); $$ = node;}
        |  modifiers_zero_or_more method_header SEMICOLON_OP                                                                                            {MethodDeclaration* node($2); node->modifiers = $1; node->addChildren({$1,$2,$3}); node->entry_type = METHOD_DECLARATION; ((LocalSymbolTable*)((global_symtab->symbol_tables)[global_symtab->current_level.first][global_symtab->current_level.second]))->add_entry(node); $$ = node;}

method_header
        :  unann_type method_declarator                                                                                                                 {MethodDeclaration* node($2); node->type = $1; node->addChildren({$1,$2}); $$ = node;}
        |  VOID_KEYWORD method_declarator                                                                                                               {Type* t = new Type("result", 8); MethodDeclaration* node($2); node->type = t; node->addChildren({$1,$2}); $$ = node;}
        // |  type_parameters unann_type method_declarator                                                                                                 {Node* node = createNode("method header"); node->addChildren({$1,$2,$3,$4}); $$ = node;}
        // |  type_parameters VOID_KEYWORD method_declarator                                                                                               {Node* node = createNode("method header"); node->addChildren({$1,$2,$3,$4}); $$ = node;}
...
//         |  FINAL_KEYWORD unann_type ELLIPSIS IDENTIFIERS                                                                                {Node* node = createNode("variable arity parameter"); node->addChildren({$1,$2,$3,$4}); $$ = node;}
...
constructor_declaration
        :  modifiers_zero_or_more constructor_declarator constructor_body                                                            {Node* node = createNode("constructor declaration"); node->addChildren({$1,$2,$3}); $$ = node;}
