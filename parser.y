...
%type<node> expression_name open_zero_or_one 
...
static_import_on_demand_declaration
            :   IMPORT_KEYWORD STATIC_KEYWORD type_name '.' '*' ';' {Node* node = createNode("static import on demand declaration"); node->addChildren({$1,$2,$3,$4,$5,$6}); $$ = node;}
...
unqualified_class_instance_creation_expression
            ...
            |   NEW_KEYWORD class_or_interface_type_to_instantiate '(' argument_list_zero_or_one ')' class_body_zero_or_one {Node* node = createNode("unqualified class instance creation expression"); node->addChildren({$1,$2,$3,$4,$5,$6}); $$ = node;}           
            |   NEW_KEYWORD type_arguments class_or_interface_type_to_instantiate '(' argument_list_zero_or_one ')' class_body_zero_or_one {Node* node = createNode("unqualified class instance creation expression"); node->addChildren({$1,$2,$3,$4,$5,$6, $7}); $$ = node;}           
...
if_then_else_statement_no_short_if
        :  IF_KEYWORD  '(' expression ')' statement_no_short_if ELSE_KEYWORD statement_no_short_if {Node* node = createNode("if then else statement no short if"); node->addChildren({$1,$2,$3,$4,$5,$6,$7}); $$ = node;}
