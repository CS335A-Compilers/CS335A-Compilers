...
expression_name
            ...
            |   expression_name  DOT_OP IDENTIFIERS  {Node* node = createNode("expression name"); node->addChildren({$1,$2,$3}); $$ = node;}           
...
switch_expression
            :   SWITCH_KEYWORD OP_BRCKT expression CLOSE_BRCKT switch_block {Node* node = createNode("cswitch expression"); node->addChildren({$1,$2,$3,$4}); $$ = node;}           

switch_block
            :   OP_CURLY_BRCKT switch_rule switch_rule_zero_or_more CLOSE_CURLY_BRCKT {Node* node = createNode("switch block"); node->addChildren({$1,$2,$3}); $$ = node;}           
            |   OP_CURLY_BRCKT switch_block_statement_group_zero_or_more switch_label_colon_zero_or_more CLOSE_CURLY_BRCKT {Node* node = createNode("switch block"); node->addChildren({$1,$2,$3}); $$ = node;}           
...
switch_block_statement_group
            :   switch_label COLON_OP switch_label_colon_zero_or_more block_statements {Node* node = createNode("switch block statement group"); node->addChildren({$1,$2,$3}); $$ = node;}           
...
comma_case_constant_zero_or_more
            ...
            |   COMMA_OP case_constant comma_case_constant_zero_or_more {Node* node = createNode("comma case constant zero or more"); node->addChildren({$1,$2}); $$ = node;}           
...
assignment_operators 
            :   ASSIGNMENT_OP {Node* node = createNode("assignment_operators"); node->addChildren({$1}); $$ = node;}           
...
dim_expr    
            :  OP_SQR_BRCKT expression CLOSE_SQR_BRCKT {Node* node = createNode("dims expr"); node->addChildren({$1,$2,$3}); $$ = node;}
...
array_initializer
            :   OP_CURLY_BRCKT variable_initializer_list_zero_or_more commas_zero_or_more CLOSE_CURLY_BRCKT {Node* node = createNode("array initializer"); node->addChildren({$1,$2,$3}); $$ = node;}
...
class_type
            ...
            |   type_name DOT_OP IDENTIFIERS type_arguments {Node* node = createNode("class type"); node->addChildren({$1,$2,$3}); $$ = node;}
            |   class_type DOT_OP IDENTIFIERS type_arguments {Node* node = createNode("class type"); node->addChildren({$1,$2,$3}); $$ = node;}
...
comma_type_arguement_zero_or_more
            ...
            |   COMMA_OP primitive_type dims comma_type_arguement_zero_or_more {Node* node = createNode("comma type arguement zero or more"); node->addChildren({$1,$2,$3}); $$ = node;}
...
wild_card
            ...
            |   QN_OP EXTENDS_KEYWORD primitive_type dims {Node* node = createNode("wild card"); node->addChildren({$1,$2,$3}); $$ = node;}
            |   QN_OP SUPER_KEYWORD reference_type  {Node* node = createNode("wild card"); node->addChildren({$1,$2,$3}); $$ = node;}
            |   QN_OP SUPER_KEYWORD primitive_type dims {Node* node = createNode("wild card"); node->addChildren({$1,$2,$3}); $$ = node;}
...
constant_declaration
            :   modifiers_zero_or_more unann_type variable_declarator_list SEMICOLON_OP {Node* node = createNode("constant decleration"); node->addChildren({$1,$2,$3}); $$ = node;}

interface_method_declaration
            :   modifiers_zero_or_more method_header block {Node* node = createNode("constant decleration"); node->addChildren({$1,$2}); $$ = node;}
            |   modifiers_zero_or_more method_header SEMICOLON_OP {Node* node = createNode("constant decleration"); node->addChildren({$1,$2}); $$ = node;}
...
comma_statement_expression_zero_or_more
        ...
        |  COMMA_OP statement_expression comma_statement_expression_zero_or_more {Node* node = createNode("comma statement expression zero or more"); node->addChildren({$1,$2}); $$ = node;}
...
break_statement
        :  BREAK_KEYWORD IDENTIFIERS SEMICOLON_OP {Node* node = createNode("break statement"); node->addChildren({$1,$2}); $$ = node;}
        |  BREAK_KEYWORD SEMICOLON_OP  {Node* node = createNode("break statement"); node->addChildren({$1,$2}); $$ = node;}

yield_statement
        :  YIELD_KEYWORD expression SEMICOLON_OP  {Node* node = createNode("yield statement"); node->addChildren({$1,$2}); $$ = node;}
...
resources_specification
        :  OP_BRCKT resource_list semicolon_zero_or_one CLOSE_BRCKT {Node* node = createNode("resources specification"); node->addChildren({$1,$2,$3}); $$ = node;}
...
field_declaration
        :  modifiers_zero_or_more unann_type variable_declarator_list SEMICOLON_OP {Node* node = createNode("field declaration"); node->addChildren({$1,$2,$3}); $$ = node;}
...
constructor_body
        ...
        |  OP_CURLY_BRCKT explicit_constructor_invocation block_statements CLOSE_CURLY_BRCKT  {Node* node = createNode("constructor body"); node->addChildren({$1,$2,$3}); $$ = node;}
...
explicit_constructor_invocation
        ...
        |  expression_name DOT_OP SUPER_KEYWORD OP_BRCKT argument_list_zero_or_one CLOSE_BRCKT SEMICOLON_OP {Node* node = createNode("explicit constructor invocation"); node->addChildren({$1,$2,$3,$4,$5,$6,$7}); $$ = node;}
        ...
        |  expression_name DOT_OP type_arguments SUPER_KEYWORD OP_BRCKT argument_list_zero_or_one CLOSE_BRCKT SEMICOLON_OP {Node* node = createNode("explicit constructor invocation"); node->addChildren({$1,$2,$3,$4,$5,$6,$7,$8}); $$ = node;}
